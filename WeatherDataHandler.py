...
class WeatherData:
    def __init__(self, dt, sunrise, sunset, moon_phase, min, max, day, morn, eve, night, humidity, id, description, pop, uvi):
        self.datetime = dt
...
class WeatherDataHandler(object):
  ...
  def _parse_data(self,jsonobject):
    ...
    for daydata in jsonobject["daily"]:
          ...
          if datetime.now() > daydata["sunrise"] and datetime.now() > daydata["sunset"]:
            weather_icon = weather_icon[1]

          return_class.append(WeatherData(
              daydata["dt"],
              daydata["sunrise"],
              daydata["sunset"],
              daydata["moon_phase"],
