...
class GoogleCalendarData(object):
  ...
  def GetCalendarData(self):
    ...
    if os.path.exists("/home/evolmonster/InkyWeatherStation/config/token.json"):
      ...
      creds = Credentials.from_authorized_user_file("/home/evolmonster/InkyWeatherStation/config/token.json", SCOPES)
    ...
    if not creds or not creds.valid:
      if creds and creds.expired and creds.refresh_token:
        ...
        flow = InstalledAppFlow.from_client_secrets_file(
            "/home/evolmonster/InkyWeatherStation/config/googlesecretdesktop.json", SCOPES
        )
        creds = flow.run_local_server(port=0)
      # Save the credentials for the next run
      with open("/home/evolmonster/InkyWeatherStation/config/token.json", "w") as token:
